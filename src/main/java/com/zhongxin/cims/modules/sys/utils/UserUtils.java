...
import com.zhongxin.cims.modules.sys.dao.AreaDao;
import com.zhongxin.cims.modules.sys.dao.MenuDao;
import com.zhongxin.cims.modules.sys.dao.OfficeDao;
import com.zhongxin.cims.modules.sys.dao.RoleDao;
import com.zhongxin.cims.modules.sys.dao.UserDao;
import com.zhongxin.cims.modules.sys.entity.Area;
...
import com.zhongxin.cims.modules.sys.security.SystemAuthorizingRealm.Principal;
...
public class UserUtils extends BaseService {
    ...
    private static LessonMapper lessonMapper = SpringContextHolder.getBean(LessonMapper.class);
    private static CourseMapper courseMapper = SpringContextHolder.getBean(CourseMapper.class);
    ...
    public static final String CACHE_EPARCHY_LIST = "eparchyList";
    public static final String CACHE_LESSON_LIST = "lessonList";
...
//			}
    ...
    public static List<Area> getAreaListByParent(String ids){
        @SuppressWarnings("unchecked")
        List<Area> areaList = (List<Area>)getCache(CACHE_EPARCHY_LIST);
        if (areaList == null){
            areaList = areaDao.findByParentIdsLike(ids);
            putCache(CACHE_EPARCHY_LIST, areaList);
        }
        return areaList;
    }
    public static String getAreaName(String id){
        ...
        List<Area> areaList = (List<Area>)getCache(CACHE_AREA_LIST);
        if (areaList == null){
            name = areaDao.get(id)==null?"":areaDao.get(id).getName();
        } else {
    ...
    public static String getCourseName(String id){
        ...
        String name = "";
        List<Course> courseList = (List<Course>)getCache(CACHE_COURSE_LIST);
        if (courseList == null){
            courseList = courseMapper.listAll();
            putCache(CACHE_COURSE_LIST,courseList);
            for(Course course:courseList){
                if (id.equals(String.valueOf(course.getCourseId()))){
    ...
    public static String getLessonName(String id){
        if (id == null || "".equals(id)){
            return "";
        }
        String name = "";
        List<Lesson> lessonList = (List<Lesson>)getCache(CACHE_LESSON_LIST);
        if (lessonList == null){
            lessonList = lessonMapper.findAll();
            putCache(CACHE_LESSON_LIST,lessonList);
            for(Lesson lesson:lessonList){
                if (id.equals(String.valueOf(lesson.getLessonId()))){
