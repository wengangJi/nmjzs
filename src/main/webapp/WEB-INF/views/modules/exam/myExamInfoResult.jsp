...
<form name="examForm" id="examForm" action="" >
    <div class="tab-pane" id="cpResume">
        ...
        <thead><tr><th>试卷信息</th></tr></thead>
        <tbody>
         <c:forEach items="${examQuesiontlist}" var="question" varStatus="status">
            <tr>
                <td  ><label class="control-label">题目${status.index+1}:${question.question}<c:if test="${question.questionStyle==0}">(单选)</c:if>
                    ...
                    <u style="color: red"> 您的答案：<strong>${question.userAnswer}</strong></u>
                     &nbsp;
                    <u style="color: red"> 正确答案： <c:forEach var="answer" items="${question.answer}" varStatus="status1"> &nbsp;<strong>${answer.answer}</strong></c:forEach>
                    </u>
                    <c:if test="${question.trueFlag==0}"><img src="${ctxStatic}/images/dui.png"/></c:if>
                    <c:if test="${question.trueFlag==1}"><img src="${ctxStatic}/images/cuo.png"/></c:if>
