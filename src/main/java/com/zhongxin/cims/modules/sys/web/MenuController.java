...
public class MenuController extends BaseController {
    ...
    public String menuTree(HttpServletRequest request, Model model) {
        model.addAttribute("extId", request.getParameter("extId"));
        return "modules/sys/sidebarMenu";
    }
    ...
    public List<Map<String, Object>> menuTreeData(@RequestParam(required=false) String extId,HttpServletRequest request, HttpServletResponse response) {
        response.setContentType("application/json; charset=UTF-8");
        List<Map<String, Object>> mapList = Lists.newArrayList();
        List<Menu> list = systemService.findTreeMenu(extId);
        for (int i=0; i<list.size(); i++){
            Menu e = list.get(i);
            if (e.getIsShow() != null && "1".equals(e.getIsShow())){
                Map<String, Object> map = Maps.newHashMap();
                map.put("id", e.getId());
                map.put("pId", e.getParent()!=null?e.getParent().getId():0);
                map.put("name", e.getName());
                map.put("href", e.getHref());
                if(e.getHref() != null && !"".equals(e.getHref())){
                    map.put("url", request.getContextPath() + Global.getAdminPath() + e.getHref());
                }
                map.put("icon",e.getIcon());
                String target = (e.getTarget() == null || e.getTarget().isEmpty())?"mainFrame":e.getTarget();
