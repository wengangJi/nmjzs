...
import com.zhongxin.cims.modules.cms.entity.Site;

import com.zhongxin.cims.modules.cms.utils.CmsUtils;
...
public class SysTreeMenu extends IdEntity<SysTreeMenu> {
       ...
       // private Office office;	// 归属部门
        ...
        private SysTreeMenu parent;// 父级菜单
    ...
    public void setAppId(String appId) {
        ...
        private List<SysTreeMenu> childList = Lists.newArrayList(); 	// 拥有子分类列表
    ...
    public SysTreeMenu(){
        ...
        public SysTreeMenu(String id){
            this();
            this.id = id;
        }

       /* public SysTreeMenu(String id, Site site){
            this();
            this.id = id;
            this.setSite(site);
        }
*/
       /* @ManyToOne(fetch = FetchType.LAZY)
        ...
        @NotFound(action = NotFoundAction.IGNORE)
        @NotNull
        public SysTreeMenu getParent() {
            return parent;
        }

        public void setParent(SysTreeMenu parent) {
            this.parent = parent;
        }
        ...
        @NotFound(action = NotFoundAction.IGNORE)
        @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
        public List<SysTreeMenu> getChildList() {
            return childList;
        }

        public void setChildList(List<SysTreeMenu> childList) {
            this.childList = childList;
        }

        @Transient
        public static void sortList(List<SysTreeMenu> list, List<SysTreeMenu> sourcelist, String parentId){
            for (int i=0; i<sourcelist.size(); i++){
                SysTreeMenu e = sourcelist.get(i);
                if (e.getParent()!=null && e.getParent().getId()!=null
                        && e.getParent().getId().equals(parentId)){
                    list.add(e);
                    // 判断是否还有子节点, 有则继续获取子节点
                    for (int j=0; j<sourcelist.size(); j++){
                        SysTreeMenu child = sourcelist.get(j);
