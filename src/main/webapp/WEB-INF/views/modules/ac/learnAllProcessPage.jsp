...
--%>
    <li><a href="#chooseLesson" data-toggle="tab">学习进度查看</a></li>
</ul>
...
<div class="tab-pane" id="chooseLesson">
    <div id="courseArea1">
        <form:form id="cpForm" modelAttribute="soPlanEntity" action="${ctx}/learn/learnAllProcess" method="post" class="breadcrumb form-search">
            ...
            <form:select id="localId" path="soPlan.localId" class="input-medium"><form:option value="" label=""/><form:options items="${fns:getDictList('LOCAL_ID')}" itemLabel="label" itemValue="value" htmlEscape="false"/></form:select>
            ...
            <label class="control-label" for="feeState">缴费状态 ：</label>
            ...
            <label class="control-label" for="hourState">学习状态 ：</label>
        ...
        <table id="lessonTable" class="table  table-bordered table-condensed table-hover">
            <thead><tr><th>申报单号</th><th>申报人</th><th>专业</th><th>培训项目</th><th>申请时间</th><th>要求学时</th><th>已学学时</th><th>缴费情况</th><th>学时情况</th><th>在线学习</th></tr></thead><tbody>

        <c:forEach items="${page.list}" var="soplan" varStatus="status">
            <tr>
                <td>${soplan.soid}</td>
                ...
                <td>
                    <c:if test="${soplan.feeState eq '1'}">
                        <a href="${ctx}/learn/learnProcessQuery?soid=${soplan.soid}&planId=${soplan.planId}">学习进度查看</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
